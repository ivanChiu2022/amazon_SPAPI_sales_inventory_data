...
report_name = f"{today_str}_AVC_SALES_REPORT"
...
sales_data = data.get("salesByAsin", [])
...
for item in sales_data:
    flattened.append({
        ...
        "asin": item.get("asin"),
        "customerReturns": item.get("customerReturns"),
        "shippedUnits": item.get("shippedUnits"),
        "shippedCogs": item.get("shippedCogs", {}).get("amount"),
        "shippedRevenue": item.get("shippedRevenue", {}).get("amount"),
        "currencyCode": item.get("shippedRevenue", {}).get("currencyCode")
    })
